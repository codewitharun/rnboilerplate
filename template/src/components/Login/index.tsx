...
const LoginAuth: React.FC<AppProps> = ({}) => {
  ...
  const onSubmit = async (credentials?: any) => {
    ...
    setLoading(true);
    try {
      const emailIdCheck = emailValidator(credentials?.userName);
      console.log('🚀 ~ onSubmit ~ emailIdCheck:', emailIdCheck);
      const passwordCheck = passwordValidator(credentials?.password);
      console.log('🚀 ~ onSubmit ~ passwordCheck:', passwordCheck);

      if (emailIdCheck || passwordCheck) {
        Alert.alert('Validation Error', emailIdCheck || passwordCheck);
        setLoading(false);
        return;
      }
